...
**Dynamic versioning**: Version comes from git tags via `hatch-vcs`. CI/release build with `uv build --no-build-isolation` after `uv sync --group dev` so hatchling/hatch-vcs come from the lockfile rather than a floating isolated env.
