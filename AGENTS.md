...
- **Self-hosted live contract:** runner label `bragerone-live`; secrets via local systemd `EnvironmentFile` (`PYBO_EMAIL`, `PYBO_PASSWORD`, `PYBO_OBJECT_ID`, `PYBO_MODULES`, optional `PYBO_LANG` / `PYBO_PLATFORM`); baselines in `/var/lib/gha/baselines/` (override with `PYBO_BASELINE_DIR`). Smoke: Actions → Live contract → `workflow_dispatch`. First run seeds the baseline; later runs fail only on structural catalog drift. Results go to issue `[live-contract] …` (separate from `[upstream-assets] …`).
