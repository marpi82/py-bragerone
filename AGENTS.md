...
Independent jobs run in parallel: `secrets` (gitleaks), `dependency-review`, `security`, `quality` (ruff check + format, mypy), `tests` (pytest 3.13), `docs-verify` (Sphinx `-W`). `build` (hatch) gates on all of them except `dependency-review` (`needs: [secrets, security, quality, tests, docs-verify]`). pip-audit runs as an artifact-producing advisory job (`continue-on-error: true`, not a blocking gate); CodeQL and OpenSSF Scorecard run separately. A scheduled **Upstream assets** workflow (`.github/workflows/upstream-assets.yml`) probes public BragerOne JS without login; it does not block PRs. Fingerprint changes post to a rolling issue titled `[upstream-assets] BragerOne public catalog fingerprint` and `repository_dispatch` **Live contract** (`.github/workflows/live-contract.yml`) onto the self-hosted runner labeled `bragerone-live` (`runs-on: [self-hosted, Linux, X64, bragerone-live]`). That job also supports manual `workflow_dispatch`, runs a library compat smoke after collect, auto-reseeds the baseline when drift is benign, and posts to a **separate** rolling issue `[live-contract] BragerOne structural catalog contract`. Credentials stay on the runner (`EnvironmentFile=/etc/gha/bragerone.env`); the first successful live run seeds `/var/lib/gha/baselines/live_contract.json` and does not fail on a missing baseline.
...
- **Self-hosted live contract:** runner label `bragerone-live`; secrets via local systemd `EnvironmentFile` (`PYBO_EMAIL`, `PYBO_PASSWORD`, `PYBO_OBJECT_ID`, `PYBO_MODULES`, optional `PYBO_LANG` / `PYBO_PLATFORM`); baselines in `/var/lib/gha/baselines/` (override with `PYBO_BASELINE_DIR`). Smoke: Actions → Live contract → `workflow_dispatch`. Structural catalog drift is **informational** (issue comment + artifact diff); the job stays green when `scripts/live_compat_smoke.py` still passes (prime → panel groups → describe/resolve). The contract step writes only a candidate JSON (`--defer-baseline`); the runner baseline is published **after** green compat for first seed, manual overwrite, or catalog-only drift (not `object_id`/`lang`/`modules`/`schema_version` changes — those need the manual checkbox). Collect/auth/parse failure or compat smoke failure keeps the previous baseline and fails the job. Results go to issue `[live-contract] …` (separate from `[upstream-assets] …`).
...
- **Sibling HA smoke (optional):** native Home Assistant lives in `ha-bragerone` (`uv run poe hass-cloud`). To exercise an unreleased library build: `USE_LOCAL_PYBRAGERONE=1 uv run poe hass-prepare` then `uv run poe hass-cloud` from that checkout (uses `--skip-pip`). Docker is not available in the current Cloud Agent image. **Live UI tests are read-only** for BragerOne entities (no writes / switches / setpoints) unless the user explicitly asks for a write.
