...
- **Sibling HA smoke (optional):** native Home Assistant lives in `ha-bragerone` (`uv run poe hass-cloud`). To exercise an unreleased library build: `USE_LOCAL_PYBRAGERONE=1 uv run poe hass-prepare` then `uv run poe hass-cloud` from that checkout (uses `--skip-pip`). Docker is not available in the current Cloud Agent image.
