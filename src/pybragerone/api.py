...
DEFAULT_TIMEOUT = ClientTimeout(total=25)
...
class Api:
    ...
    async def list_objects(self) -> list[Any]:
        objs: list[dict[str, JSON]] = []
        try:
    ...
    async def list_modules(self, object_id: int) -> list[Any]:
    ...
    async def snapshot_parameters(self, devs: list[str]) -> Any:
        res = await self._req("POST", f"{API_BASE}/modules/parameters", json={"modules": devs})
        return res if isinstance(res, dict) else {}

    async def activity_quantity(self, devs: list[str]) -> Any:
        res = await self._req(
