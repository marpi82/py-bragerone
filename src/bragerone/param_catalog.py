...
- param_units: mapowanie z runtime (po snapshot’cie): (pool, id) → unit_id (jako string)
...
class ParamCatalog:
    ...
    def __init__(self, units: dict, labels: dict, meta: dict):
        self.units = units or {}
        self.labels_by_param = labels or {}
        self.meta_by_name = meta or {}
        self.param_units = {}  # (pool, param_id) -> unit_id_str (runtime)
    ...
    def describe_param(self, param_id: int) -> str | None:
        ...
        return self.labels_by_param.get(key)
    ...
    def pretty_param_key(self, pool: str, var: str) -> str:
        ...
        if var and (
            ...
            if name in self.meta_by_name:
                return name
            return f"parameters.PARAM_{pid}"
        return f"{pool}.{var}"

    # ---- formatowanie wartości ----
    def _lookup_unit(self, unit_id: str) -> Any:
